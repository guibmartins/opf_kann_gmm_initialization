...
class ANNSubgraph(Subgraph):
    ...
    def __init__(self, X=None, Y=None, I=None, from_file=None, pdf_fn='gaussian', weight_pdf=False):
        ...
        # An indicator of the pdf function to be used
        self.pdf_function = pdf_fn

        # Whether or not to weight the pdf calculation
    ...
    def pdf_function(self):
        ...
        return self._pdf_function

    @pdf_function.setter
    def pdf_function(self, pdf_function):

        if not isinstance(pdf_function, str):
            raise e.TypeError('pdf_function should be a string')

        if pdf_function == 'gaussian':
            self._pdf_function = self._gaussian_kernel
        elif pdf_function == 'local':
            self._pdf_function = self._local_density
        else:
            raise e.ValueError('pdf_function should be a valid key.')

    @property
    def weight_pdf(self):

        return self._weight_pdf

    @weight_pdf.setter
    def weight_pdf(self, weight_pdf):

        if not isinstance(weight_pdf, bool):
            raise e.TypeError('`weight_pdf` should be a boolean')

        self._weight_pdf = weight_pdf

    def _gaussian_kernel(self, n_neighbors):
        """Calculates the gaussian probability density function for `k` neighbours.

        Args:
            n_neighbors (int): Number of neighbours in the adjacency relation.
        ...
        pdf = np.empty((self.n_nodes,), dtype=float)

        # For every possible node
        for i in range(self.n_nodes):

            # Initialize the number of p.d.f. calculations as 1
            n_pdf = 1 + len(self.nodes[i].adjacency[:n_neighbors])

            # For every possible `k` value, calculate the p.d.f.
            pdf[i] = np.sum(np.exp(-np.asfarray(self.nodes[i].adj_distances[:n_neighbors]) / self.constant))
    ...
    def _local_density(self, n_neighbors):

        pdf = np.empty((self.n_nodes,), dtype=float)

        for i in range(self.n_nodes):
            pdf[i] = 1. / (np.sum(self.nodes[i].adj_distances[:n_neighbors]) + c.EPSILON)

        return pdf

    def _relative_dist(self, density, distance_function, ann_search):

        delta = np.empty((self.n_nodes,), dtype=float)

        for i in range(self.n_nodes):

            # Query densities that are higher than the current one
            idx_higher_dens = np.flatnonzero(density > density[i])

            # If value is the global density peak
            if not len(idx_higher_dens):

                # Search for n neighbors, i.e., all data points
                _, distances = ann_search.query(self.nodes[i].features, self.n_nodes)

                # Get the maximum distance among all data
                delta[i] = float(distances[-1])

            else:

                distances = [
                    distance_function(self.nodes[i].features, self.nodes[j].features)
                    for j in idx_higher_dens]

                # Get the minimum distance with respect to higher densities
                delta[i] = float(np.min(distances))
    ...
    def calc_pdf(self, n_neighbors, distance_function, ann_search):
        """Calculates the probability density function for `k` neighbours.

        Args:
            n_neighbors (int): Number of neighbours in the adjacency relation.
            distance_function (callable): The distance function to be used to calculate the arcs.
            ann_search (ApproximateNNSearch): Instance of the ann index constructed over the subgraph.

        """

        # Calculate density (pdf) for each graph node using a k-neighborhood Gaussian kernel
        density = self.pdf_function(n_neighbors)

        delta = 1

        if self.weight_pdf:
            # Calculate the relative distances that will be used to weight each corresponding node's density
            delta = self._relative_dist(density, distance_function, ann_search)
        ...
        # If subgraph's minimum density is different than the maximum density
            ...
            for i in range(self.n_nodes):
                ...
                # Calculates the node's density
                self.nodes[i].density = (
                    float(c.MAX_DENSITY - 1) * (pdf[i] - self.min_density) / (self.max_density - self.min_density)) + 1.
    ...
    def build_arcs(self, k, ann_search):
        ...
        for i in range(self.n_nodes):
            ...
            # Search for (k + 1) neighbors (the first idx is the node i itself)
            ...
            # Also make sure that it doesn't have any adjacent nodes (only for min cut)
            self.nodes[i].n_plateaus = 0

            # Apply maximum distance as the distance
            max_distances = np.maximum(max_distances, distances[1:])

            # Adds neighbours to the adjacency list of node `i`
            self.nodes[i].adjacency.extend(neighbors_idx[1:])

            # Adds neighbours distances to the adjacency list of node `i`
